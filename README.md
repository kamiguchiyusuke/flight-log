...
`localStorage` はセッションが終わると消える（そもそも書けないこともある）ので、
毎回 `#` 付きで開くことになる。書き込みは `try/catch` で囲んであるため、
保存できない環境でも壊れない。

この構成は github.io → script.google.com → googleusercontent.com と
**異なるドメインが 3 層**になる。Safari のトラッキング防止がクロスサイトの
ストレージを遮断するので `google.script.run` が弾かれる懸念があったが、
**プライベートモードで記録を保存できることを実機で確認済み**。
匿名アクセス（ログイン不要）なので Cookie に依存しないためと思われる。
