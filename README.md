...
`AIRLINE_CODES` の全社と、搭乗履歴・`airlines` シートに出てくる会社を一覧にする。
搭乗回数の多い順に並ぶので、よく乗る会社が上に来る。

**行をタップするとロゴを取得・取消する。** まだ乗っていない航空会社もここから入れられる。
台帳の印は「その会社が台帳に 1 行でも出ている」のが前提なので、初めて乗る前に
ロゴを用意したいときはこのタブを使う。

行の下に出る `NH` などのコードに「使わない」が付いていれば、
その会社は保存時の自動取得を止めてある状態（後述の `-`）。タップすれば取りに行く。
...
「台帳」タブには全件が搭乗日の新しい順に並ぶ。年ごとに見出しが入り、
上部に総フライト数とユニーク機体数が出る。

**台帳の操作はすべてスマホで完結する。スプレッドシートを開く必要はない。**
...
編集中の照合は、その記録自身を搭乗回数から外して数える。
3 回目の記録を直しているのに 4 回目と出ないようにするため。
