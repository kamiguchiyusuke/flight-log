...
行は 3 列。左にロゴ、真ん中に区間、右端に機材。日付は空港名の下。

```
[ANA]  成田(NRT) ────────── 関西(KIX)      JA824J
       09.21 · JL123 · 日本航空            B787-8
```

真ん中の列は**出発と到着を両端へ振り、間に飛行機を置く。** 空港名は長さが
まちまち（`羽田(HND)` と `台北桃園(TPE)`）なので、両端は縮んで折り返せるように
してあり、余った幅を飛行機の欄が吸う。片側しか無い記録では飛行機を出さない。

飛行機は自前の SVG。絵文字は端末ごとに形も向きも変わるので使わない。
形の定義は `<symbol id="ic-plane">` に 1 回だけ置き、各行は `<use>` で参照する。

360px 未満では空港名が毎行折り返すので、その幅でだけ文字を一段落とす。
