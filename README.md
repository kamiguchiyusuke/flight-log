...
```
[ANA]  成田(NRT)     ✈     関西(KIX)     B787-8
              09.21 · JL123
```

日付と便名は飛行機の真下（中央寄せ）。航空会社名は書かない。左端のロゴが
同じことを言っているため。ただしロゴも符号も出せない行だけは名前を残す。

右端は登録記号ではなく型式。記録では機材が何かを見たいのであって、
どの個体だったかはその場で見返すものではない。登録記号はシートには入る。

行の要素は**空港名の 1 行目を基準の線として揃える。** ロゴも飛行機も型式も、
